...
export default function Sidebar({ page }) {
    ...
    const isStudent = user.roles.find((e) => {
        return e === "student";
    })
        ? true
        : false;
    const isAdmin = user.roles.find((e) => {
        return e === "admin";
    })
        ? true
        : false;
    const isTeacher = user.roles.find((e) => {
        return e === "teacher";
    })
        ? true
        : false;
    ...
    }, [sidebarToggle]); // Re-run effect when sidebarToggle state changes

    const toggleSidebar = () => {
        // Toggle the sidebarToggle state
        setSidebarToggle(!sidebarToggle);
    };

    const handleNavigation = (pageName) => {
        // Toggle the selected state based on current state
        setSelected(selected === pageName ? "" : pageName);
    };

    return (
        <aside className="absolute left-0 top-0 z-9999 flex h-screen w-72.5 flex-col overflow-y-hidden gradient-v duration-150 ease-in-out dark:bg-boxdark lg:static lg:translate-x-0">
            {/* <!-- SIDEBAR HEADER --> */}
            <div className="p-4 flex items-center justify-between gap-2 px-6 py-5.5 lg:py-6.5">
                <a
                    href="index.html"
                    className="flex items-center justify-center gap-4 text-bodydark1 font-extrabold text-3xl"
                >
                    <ApplicationLogo className={`w-10 inline`} />
                    <span>
                        Sharing <span>Space</span>
                    </span>
                </a>
            ...
            <div className="no-scrollbar flex flex-col overflow-y-auto duration-150 ease-in-out h-full">
                {/* <!-- Sidebar Menu --> */}
                <nav
                    className="mt-5 px-4 py-4 lg:mt-9 lg:px-6 h-full"
                    x-data="{selected: $persist('Dashboard')}"
                >
                    {/* <!-- Menu Group --> */}
                    <div className="w-full h-full flex flex-col">
                        <ul className="mb-6 flex flex-1 flex-col gap-1.5 justify-self-start">
                            {isStudent && (
                                /* Menu Item Home */
                                <li>
                                    <a
                                        className={`group relative flex items-center gap-2.5 rounded-lg px-4 py-2 font-bold text-bodydark1 duration-150 ease-in-out border-meta-4 hover:bg-meta-2 hover:bg-opacity-50 hover:border-l-4 hover:text-meta-4 dark:hover:bg-meta-4
                            ...
                            {isAdmin || isTeacher ? (
                                <li>
                                    ...
                                    // :className="{ 'bg-graydark dark:bg-meta-4': (selected === 'Dashboard') || (page === 'ecommerce' || page === 'analytics' || page === 'stocks') }"
                                    > */}
                                    <a
                                        className={`group relative flex items-center gap-2.5 rounded-lg px-4 py-2 font-bold text-bodydark1 duration-150 ease-in-out border-meta-4 
      ${selected === "Dashboard" ? "bg-graydark dark:bg-meta-4" : ""}`}
                                        href={route("dashboard")}
                                        onClick={(e) => {
                                            e.preventDefault(); // Prevent default link behavior
                                            handleNavigation("Dashboard"); // Handle navigation and state update
                                        }}
                                    >
                                        <svg
                                            className="fill-current"
                                        ...
                                        Dashboard
                                        <svg
                                            className={`absolute right-4 top-1/2 -translate-y-1/2 fill-current transition-all duration-200 ease-in-out
                                                    ${
                                                        selected ===
                                                            "Dashboard" &&
                                                        "rotate-180"
                                    ...
                                    >
                                        <ul className="mb-5.5 mt-4 flex flex-col gap-2.5 pl-6">
                                            <li>
                                                <a
                                                    className="group relative flex items-center gap-2.5 rounded-md px-4 font-medium text-slate-200 duration-150 ease-in-out hover:text-slate-800"
                                                    href="index.html"
                            ...
                            <li>
                                <a
                                    className={`group relative flex items-center gap-2.5 rounded-lg px-4 py-2 font-bold text-bodydark1 duration-150 ease-in-out border-meta-4 hover:bg-meta-2 hover:bg-opacity-50 hover:border-l-4 hover:text-meta-4 dark:hover:bg-meta-4
                                    ${selected === 'ChatBox' && page === 'ChatBox' && 'bg-graydark dark:bg-meta-4' }`}
                                    href={route("chats")}
                                    onClick={(e) => {
                                        handleNavigation('ChatBox');
                                    }}
                        ...
                        <ul className="mb-6 flex flex-col gap-1.5 justify-self-end">
                            {/* <!-- Menu Item Settings --> */}
                            <li>
                                <a
                                    className="group relative flex items-center gap-2.5 rounded-lg px-4 py-2 font-bold text-bodydark1 duration-150 ease-in-out border-meta-4 hover:bg-meta-2 hover:bg-opacity-50 hover:border-l-4 hover:text-meta-4 dark:hover:bg-meta-4"
